...
function CSSJanus() {
	...
	var
		...
		suffixRegExp = new RegExp( '(\\s*(?:!important)?\\s*)$', 'gi' );
	...
	function calculateNewBorderRadius( match, pre, values ) {
		var suffix, m, numValues;

		m = values.match( suffixRegExp );
		if ( m && m[0] ) {
			suffix = m[0];
			values = values.slice( 0, -suffix.length );
		} else {
			suffix = '';
		}

		numValues = values.trim().split( /\s+/g ).length;
		switch ( numValues ) {
			case 4:
